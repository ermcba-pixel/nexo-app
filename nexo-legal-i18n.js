    ...
    <style>
        * {
        ...
        .cart-item {
            ...
            padding: 14px 16px;
            display: grid;
            grid-template-columns: 120px minmax(240px, 1fr) 130px 120px;
            align-items: center;
            gap: 16px;
        ...
        @media (max-width: 768px) {
            ...
            .cart-item-icon{width:82px;height:78px;}
            .cart-item-costs,.cart-item-actions{grid-column:1 / -1;}
        ...
        .cart-item-features{font-size:10px;color:#4A6B6D;line-height:1.4;background:#f5fbf9;border-radius:6px;padding:7px;margin-top:2px;}
...
<body>
    ...
    <div class="header">
        ...
        <button class="back-btn" onclick="window.location.href='nexo-tienda-cliente.html'"><span id="backStore">← <span id="confirmBackText">Volver</span> a Tienda</span></button>
    </div>
    ...
    <div class="container">
        ...
        <div class="summary-section">
            ...
            <div class="summary-row shipping">
                <span><span id="sumShipLbl">Gastos de Envío CJ Dropshipping:</span></span>
                <span class="summary-value" id="amazonShipping">$0.00</span>
            </div>
            <div class="summary-row shipping">
                <span><span id="sumVendorLbl">Costos operativos CJ:</span></span>
...
function applyCheckoutLanguage(){
 ...
 document.querySelectorAll('.payment-method, .payment-btn').forEach(btn=>{
   ...
   if(raw.includes('tarjeta')||raw.includes('card')||raw.includes('credito')) btn.innerHTML='💳 '+({es:'Tarjeta de Crédito/Débito',en:'Credit/Debit Card',pt:'Cartão de Crédito/Débito',it:'Carta di Credito/Debito',fr:'Carte de Crédit/Débit'}[currentLang]||'Credit/Debit Card');
        ...
        // MÉTODOS DE PAGO
        // Todos los métodos visibles. Tarjeta NO redirige a PayPal: queda en captura directa/Agente 1.
        ...
        function renderCountrySelect() {
            ...
            select.innerHTML = '<option value=""></option>'; const cg=document.getElementById('countryGroup'); if(cg) cg.style.display='none';
...
function nexoPayText(key){
    const lang = (localStorage.getItem('nexoLang') || localStorage.getItem('nexo_language') || currentLang || 'es').slice(0,2);
    const D = {
      es:{
        cardTitle:'Datos de tarjeta de crédito/débito',
        cardText:'Pago con tarjeta dentro de nexo™. No se redirige al cliente a PayPal ni se exige cuenta PayPal. nexo™ no guarda CVV ni tarjeta completa; registra la intención para captura directa del Agente 1/pasarela aprobada.',
        cardBtn:'Pagar con tarjeta en nexo',
        bankTitle:'Transferencia bancaria / ACH / Wire',
        bankText:'Registre los datos bancarios o referencia para seguimiento y conciliación. La información se guarda protegida en Supabase; NO se emite factura pagada hasta confirmar el abono bancario.',
        bankHolder:'Titular de la cuenta bancaria', bankName:'Banco del cliente', bankNamePh:'Ej.: Bank of America, BNB, banco del cliente, etc.', bankRef:'Referencia / número de operación (opcional)', transferType:'Tipo de transferencia', bankBtn:'Continuar con transferencia / pago seguro',
        paypalBtn:'Continuar con PayPal', payoneerText:'Al confirmar, nexo™ abrirá el checkout autorizado de Payoneer cuando esté configurado en Vercel. Si aún no existe URL/API de Payoneer, el pedido quedará registrado para completar integración.', payoneerBtn:'Pagar con Payoneer'
      },
      en:{
        cardTitle:'Direct card payment in nexo', cardText:'nexo™ does not redirect the customer to PayPal or require a PayPal account. Card capture is registered for direct gateway/Agent 1 processing. nexo™ does not store CVV or full card numbers.', cardBtn:'Pay by card in nexo',
        bankTitle:'Bank transfer / ACH / Wire', bankText:'Register the customer bank or reference for tracking. The real charge will be processed through secure PayPal if PayPal enables bank account/ACH/Wire; if using a direct transfer, the order remains pending until funds are confirmed.', bankHolder:'Bank account holder', bankName:'Customer bank', bankNamePh:'Ex.: Bank of America, local bank, etc.', bankRef:'Reference / operation number (optional)', transferType:'Transfer type', bankBtn:'Continue with transfer / secure payment', paypalBtn:'Continue with PayPal', payoneerText:'', payoneerBtn:'Continue with Payoneer'
      },
      pt:{cardTitle:'Pagamento real com cartão via PayPal seguro', cardText:'nexo™ não guarda números de cartão nem CVV. Ao confirmar, o PayPal Business será aberto para pagar de forma real e segura com PayPal ou cartão, conforme disponibilidade do PayPal no país do cliente.', cardBtn:'Continuar com cartão / PayPal seguro', bankTitle:'Transferência bancária / ACH / Wire', bankText:'Registre o banco ou referência do cliente para acompanhamento. A cobrança real será processada via PayPal seguro se o PayPal habilitar conta bancária/ACH/Wire; se usar transferência direta, o pedido fica pendente até confirmar o crédito.', bankHolder:'Titular da conta bancária', bankName:'Banco do cliente', bankNamePh:'Ex.: Bank of America, local bank, etc.', bankRef:'Referência / número da operação (opcional)', transferType:'Tipo de transferência', bankBtn:'Continuar com transferência / pagamento seguro', paypalBtn:'Continuar com PayPal', payoneerText:'', payoneerBtn:'Continuar com Payoneer'},
      it:{cardTitle:'Pagamento reale con carta tramite PayPal sicuro', cardText:'nexo™ non conserva numeri di carta né CVV. Alla conferma, si aprirà PayPal Business per pagare in modo reale e sicuro con PayPal o carta, secondo la disponibilità di PayPal nel paese del cliente.', cardBtn:'Continua con carta / PayPal sicuro', bankTitle:'Bonifico bancario / ACH / Wire', bankText:'Registra la banca o il riferimento del cliente per il monitoraggio. L’addebito reale sarà processato tramite PayPal sicuro se PayPal abilita conto bancario/ACH/Wire; se usa un bonifico diretto, l’ordine resta in sospeso fino alla conferma dell’accredito.', bankHolder:'Intestatario del conto bancario', bankName:'Banca del cliente', bankNamePh:'Es.: Land Bank, Bank of America, ecc.', bankRef:'Riferimento / numero operazione (opzionale)', transferType:'Tipo di bonifico', bankBtn:'Continua con bonifico / pagamento sicuro', paypalBtn:'Continua con PayPal', payoneerText:'', payoneerBtn:'Continua con Payoneer'},
      fr:{cardTitle:'Paiement réel par carte via PayPal sécurisé', cardText:'nexo™ ne conserve ni numéros de carte ni CVV. À la confirmation, PayPal Business s’ouvrira afin que le client paie réellement et en sécurité avec PayPal ou carte, selon la disponibilité PayPal dans son pays.', cardBtn:'Continuer avec carte / PayPal sécurisé', bankTitle:'Virement bancaire / ACH / Wire', bankText:'Enregistrez la banque ou la référence du client pour le suivi. Le débit réel sera traité via PayPal sécurisé si PayPal active compte bancaire/ACH/Wire ; en cas de virement direct, la commande reste en attente jusqu’à confirmation des fonds.', bankHolder:'Titulaire du compte bancaire', bankName:'Banque du client', bankNamePh:'Ex. : Land Bank, Bank of America, etc.', bankRef:'Référence / numéro d’opération (facultatif)', transferType:'Type de virement', bankBtn:'Continuer avec virement / paiement sécurisé', paypalBtn:'Continuer avec PayPal', payoneerText:'', payoneerBtn:'Continuer avec Payoneer'}
    };
    return (D[lang] || D.es)[key] || D.es[key] || key;
}

function nexoLegalBlockPage(){
    return `
      <div class="nexo-checkout-legal page-legal-block" style="margin:18px 0;padding:18px 20px;border:1px solid rgba(74,107,109,.28);border-radius:16px;background:#f6faf9;color:#2D3E3F;font-size:14px;line-height:1.6;">
        <p style="margin:0 0 8px 0;"><strong>Condiciones legales de compra nexo™</strong></p>
        <p style="margin:0 0 12px 0;">nexo™ actúa como intermediario comercial internacional. El cliente acepta que el pago cubre producto, logística, costos del proveedor y comisión nexo™. Garantías, cambios, devoluciones y tiempos de entrega dependen del proveedor y de la logística internacional.</p>
        <p style="margin:0 0 12px 0;"><a href="terminos-condiciones.html" target="_blank">Términos y Condiciones</a> | <a href="politica-privacidad.html" target="_blank">Privacidad</a> | <a href="reembolsos.html" target="_blank">Reembolsos</a></p>
        <label style="display:flex;gap:10px;align-items:flex-start;font-weight:900;cursor:pointer;">
          <input type="checkbox" id="nexoLegalAcceptPage" style="margin-top:3px;min-width:20px;min-height:20px;" onchange="const m=document.getElementById('nexoLegalAccept'); if(m)m.checked=this.checked;">
          <span>Acepto los términos legales, privacidad, pagos, reembolsos y condición de intermediación comercial internacional de nexo™.</span>
        </label>
      </div>`;
}
function nexoSyncLegalFromPageToModal(){
    const page = document.getElementById('nexoLegalAcceptPage');
    const modal = document.getElementById('nexoLegalAccept');
    if(page && modal) modal.checked = !!page.checked;
}


function nexoMaskedDigits(v){ return String(v||'').replace(/\s+/g,'').replace(/.(?=.{4})/g,'•'); }
function nexoLast4(v){ const d=String(v||'').replace(/\D/g,''); return d.slice(-4); }
function nexoCardBrand(v){ const d=String(v||'').replace(/\D/g,''); if(/^4/.test(d)) return 'Visa'; if(/^5[1-5]/.test(d)) return 'Mastercard'; if(/^3[47]/.test(d)) return 'American Express'; if(/^6/.test(d)) return 'Discover'; return 'Tarjeta'; }
function nexoPaymentProfileKey(){
  const prof = JSON.parse(sessionStorage.getItem('nexoActiveClient') || localStorage.getItem('nexoClientProfile') || '{}');
  return 'nexoPaymentProfile_' + String(prof.email || prof.documentId || prof.documento || 'guest').toLowerCase().replace(/[^a-z0-9@._-]/g,'_');
}
function nexoLoadPaymentProfile(){ try{return JSON.parse(localStorage.getItem(nexoPaymentProfileKey())||'{}')}catch(e){return {}} }
function nexoSavePaymentProfileLocal(data){
  const prev=nexoLoadPaymentProfile();
  const merged={...prev,...data,updatedAt:new Date().toISOString(),cvvStored:false};
  localStorage.setItem(nexoPaymentProfileKey(), JSON.stringify(merged));
  return merged;
}
function nexoPrefillPaymentFields(methodId){
  const p=nexoLoadPaymentProfile();
  if(methodId==='bank' && p.bank){
    const b=p.bank;
    if(document.getElementById('bankHolder')) document.getElementById('bankHolder').value=b.holder||'';
    if(document.getElementById('bankName')) document.getElementById('bankName').value=b.bankName||'';
    if(document.getElementById('bankRouting')) document.getElementById('bankRouting').value=b.routingNumber||'';
    if(document.getElementById('bankAccount')) document.getElementById('bankAccount').value=b.accountMasked||'';
    if(document.getElementById('bankReference')) document.getElementById('bankReference').value=b.reference||'';
    if(document.getElementById('transferType')) document.getElementById('transferType').value=b.rail||'ACH';
  }
  if(methodId==='card' && p.card){
    const c=p.card;
    if(document.getElementById('cardHolder')) document.getElementById('cardHolder').value=c.holder||'';
    if(document.getElementById('cardNumber')) document.getElementById('cardNumber').value=c.maskedNumber||'';
    if(document.getElementById('cardExpiry')) document.getElementById('cardExpiry').value=c.expiry||'';
    if(document.getElementById('cardCvv')) document.getElementById('cardCvv').value='';
  }
}
async function nexoSavePaymentProfileServer(profile){
  try{
    await fetch('/api/nexo-save-payment-profile',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(profile)});
  }catch(e){ console.warn('Perfil de pago pendiente de sincronizar en Supabase', e); }
}
...
function selectPayment(methodId, btnEl=null) {
    ...
    if (methodId === 'card') {
        details.innerHTML = `
            <div class="payment-fields-panel">
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="cardHolder" name="nexo_pago_ref_1" placeholder="Nombre del titular" autocomplete="new-password" autocorrect="off" spellcheck="false" data-lpignore="true" data-form-type="other" readonly onfocus="this.removeAttribute('readonly')">
                </div>
                <div class="form-group">
                    <label class="form-label">NÚMERO DE TARJETA</label>
                    <input type="text" class="form-input" id="cardNumber" name="nexo_pago_ref_2" placeholder="Número de tarjeta" inputmode="numeric" autocomplete="new-password" autocorrect="off" spellcheck="false" data-lpignore="true" data-form-type="other" readonly onfocus="this.removeAttribute('readonly')">
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    <div class="form-group">
                        <label class="form-label">MM/AA</label>
                        <input type="text" class="form-input" id="cardExpiry" name="nexo_pago_ref_3" placeholder="MM/AA" autocomplete="new-password" autocorrect="off" spellcheck="false" data-lpignore="true" data-form-type="other" readonly onfocus="this.removeAttribute('readonly')">
                    </div>
                    <div class="form-group">
                        <label class="form-label">CVV</label>
                        <input type="password" class="form-input" id="cardCvv" name="nexo_pago_ref_4" placeholder="CVV" autocomplete="new-password" autocorrect="off" spellcheck="false" data-lpignore="true" data-form-type="other" readonly onfocus="this.removeAttribute('readonly')">
                    </div>
                </div>
                ${nexoLegalBlockPage()}
                <button type="button" class="checkout-btn" style="margin-top:10px;" onclick="openConfirmationModal()">${nexoPayText('cardBtn')}</button>
            </div>
        `;
    } else if (methodId === 'bank') {
        details.innerHTML = `
            <div class="payment-fields-panel">
                <div style="background:#f4f9ef;border-left:4px solid #7AC742;border-radius:10px;padding:14px;font-size:13px;line-height:1.55;margin-bottom:12px;">
                    <strong>${nexoPayText('bankTitle')}</strong><br>${nexoPayText('bankText')}
                </div>
                <div class="form-group">
                    <label class="form-label">${nexoPayText('bankHolder')}</label>
                    <input type="text" class="form-input" id="bankHolder" placeholder="${nexoPayText('bankHolder')}">
                </div>
                <div class="form-group">
                    <label class="form-label">${nexoPayText('bankName')}</label>
                    <input type="text" class="form-input" id="bankName" placeholder="${nexoPayText('bankNamePh')}">
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    <div class="form-group">
                        <label class="form-label">Routing / ABA / SWIFT</label>
                        <input type="text" class="form-input" id="bankRouting" placeholder="Routing / ABA / SWIFT">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Account / Cuenta</label>
                        <input type="text" class="form-input" id="bankAccount" placeholder="Account / Cuenta">
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label">${nexoPayText('bankRef')}</label>
                    <input type="text" class="form-input" id="bankReference" placeholder="${nexoPayText('bankRef')}">
                </div>
                <div class="form-group">
                    <label class="form-label">${nexoPayText('transferType')}</label>
                ...
                ${nexoLegalBlockPage()}
                <button type="button" class="checkout-btn" style="margin-top:10px;" onclick="openConfirmationModal()">${nexoPayText('bankBtn')}</button>
            </div>
        `;
        ...
        details.innerHTML = `
            <div class="payment-fields-panel">
                ${nexoLegalBlockPage()}
                <button type="button" class="checkout-btn" style="margin-top:10px;" onclick="openConfirmationModal()">${nexoPayText('payoneerBtn')}</button>
            </div>
        `;
    } else {
        details.innerHTML = `
            <div class="payment-fields-panel">
                ${nexoLegalBlockPage()}
                <button type="button" class="checkout-btn" style="margin-top:10px;" onclick="openConfirmationModal()">${nexoPayText('paypalBtn')}</button>
            </div>
        `;
    }
}
        ...
        function loadCart() {
            ...
            container.innerHTML = cart.map((rawItem, idx) => {
                ...
                item.shippingAmazon = Number(item.cjShippingCost ?? item.shippingAmazon ?? item.envio ?? item.costo_envio_usd ?? 0);
                item.vendorFee = Number(item.cjHandlingFee ?? item.vendorFee ?? item.gasto_vendedor ?? item.vendor_fee ?? 0);
                if (typeof estimateCjShipping === 'function' && (!item.shippingAmazon || item.shippingAmazon <= 0)) item.shippingAmazon = estimateCjShipping(item);
                if (typeof estimateCjHandling === 'function' && (!item.vendorFee || item.vendorFee <= 0)) item.vendorFee = estimateCjHandling(item);
                ...
                <div class="cart-item">
                    ...
                    <div class="cart-item-main">
                        ...
                        ${item.features ? `<div class="cart-item-features">${item.features}</div>` : ''}
                    </div>
        ...
        // CALCULAR TOTALES
        function getItemProductCost(item){ return Number(item.cjProductCost ?? item.price ?? 0) * Number(item.quantity ?? 1); }
        function isCjItem(item){
            const src = String(item.provider || item.proveedor || item.vendor || item.source || item.category || '').toLowerCase();
            return src.includes('cj') || src.includes('dropshipping') || !!(item.cjProductId || item.pid || item.cjSku || item.variantSku);
        }
        function getRealShippingValue(item){ return Number(item.cjShippingCost ?? item.shippingAmazon ?? item.shippingCost ?? item.envio ?? item.costo_envio_usd ?? 0) || 0; }
        function getRealVendorValue(item){ return Number(item.cjHandlingFee ?? item.vendorFee ?? item.handlingFee ?? item.costo_operativo_cj ?? 0) || 0; }
        function estimateCjShipping(item){
            // Estimación operativa temporal: se reemplaza automáticamente por la cotización real de CJ cuando la API devuelve flete por país/dirección.
            const unit = Number(item.cjProductCost ?? item.price ?? 0) || 0;
            const qty = Number(item.quantity ?? 1) || 1;
            if(!isCjItem(item)) return 0;
            return Number(Math.max(3.90, unit * 0.45, 2.40 * qty).toFixed(2));
        }
        function estimateCjHandling(item){
            const unit = Number(item.cjProductCost ?? item.price ?? 0) || 0;
            const qty = Number(item.quantity ?? 1) || 1;
            if(!isCjItem(item)) return 0;
            return Number(Math.max(0.75, unit * qty * 0.08).toFixed(2));
        }
        function getItemShippingCost(item){ const real=getRealShippingValue(item); return (real>0 ? real : estimateCjShipping(item)) * Number(item.quantity ?? 1); }
        function getItemVendorCost(item){ const real=getRealVendorValue(item); return (real>0 ? real : estimateCjHandling(item)) * Number(item.quantity ?? 1); }

        // CALCULAR TOTALES
        function calculateTotals() {
            const cart = JSON.parse(localStorage.getItem('nexoCart') || '[]');
            const subtotal = cart.reduce((total, item) => total + getItemProductCost(item), 0);
            // Costos CJ reales: no se agregan recargos inventados al cambiar estándar/prioritario.
            // Si CJ devuelve costo de envío/costo operativo, se usa exactamente ese valor.
            const amazonShipping = cart.reduce((total, item) => total + getItemShippingCost(item), 0);
            const vendorFees = cart.reduce((total, item) => total + getItemVendorCost(item), 0);
...
function openConfirmationModal() {
            ...
            const subtotal = cart.reduce((sum, item) => sum + getItemProductCost(item), 0);
            const amazonShipping = cart.reduce((sum, item) => sum + getItemShippingCost(item), 0);
            const vendorFees = cart.reduce((sum, item) => sum + getItemVendorCost(item), 0);
            const companyInfo = JSON.parse(localStorage.getItem('nexoCompanyInfo') || '{}');
            const commissionRate = typeof companyInfo.commission === 'number' ? companyInfo.commission / 100 : (parseFloat(localStorage.getItem('nexoCommission') || '30') / 100);
            const commission = subtotal * commissionRate;
            const itfRate = typeof companyInfo.itf === 'number' ? companyInfo.itf / 100 : 0.03;
...
const t = ctt();
        ...
        async function startNexoPagoSeguro(orderData){
            ...
            // El Agente 1 espera confirmación/captura PayPal y luego prepara la compra CJ Dropshipping.
            const amount = Number(orderData.total || 0).toFixed(2);
            const pedidoId = orderData.supabase_id || orderData.id || ('NEXO-' + Date.now());
            const successUrl = `${window.location.origin}/nexo-confirmacion.html?pedido_id=${encodeURIComponent(pedidoId)}&invoice=${encodeURIComponent(orderData.factura_numero || pedidoId)}&metodo=${encodeURIComponent(orderData.paymentMethod || 'paypal')}&paypal=return`;
            const cancelUrl = `${window.location.origin}/nexo-checkout.html?pedido_id=${encodeURIComponent(pedidoId)}&pago=cancelado&metodo=${encodeURIComponent(orderData.paymentMethod || 'paypal')}`;
            ...
            try {
                const r = await fetch('/api/paypal-create-order', {
                    ...
                    body: JSON.stringify({
                        ...
                        fundingSource: orderData.paymentMethod === 'card' ? 'card' : 'paypal',
...
function nexoIsAmazonItem(item){
  const txt = String([item?.provider,item?.proveedor,item?.vendor,item?.source,item?.url,item?.sourceUrl].join(' ')).toLowerCase();
  return txt.includes('amazon');
}
function nexoAmazonUrlFromOrder(order){
  const tag='nexo08-20';
  const items=Array.isArray(order?.items)?order.items:[];
  const first=items.find(nexoIsAmazonItem) || items[0] || {};
  let raw = first.sourceUrl || first.url || first.originalProviderUrl || '';
  try{
    let u = raw && String(raw).startsWith('http') ? new URL(raw) : new URL('https://www.amazon.com/s?k='+encodeURIComponent(first.name||first.title||'producto'));
    u.searchParams.set('tag', tag);
    return u.toString();
  }catch(e){
    return 'https://www.amazon.com/s?k='+encodeURIComponent(first.name||first.title||'producto')+'&tag='+encodeURIComponent(tag);
  }
}
async function nexoRegisterAmazonAffiliateClick(order){
  try{ await fetch('/api/amazon-affiliate-click',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({order, amazon_tag:'nexo08-20', product_url:nexoAmazonUrlFromOrder(order)})}); }catch(e){}
}
        ...
        async function submitPurchase(){
            ...
            const legalAccept = document.getElementById('nexoLegalAccept');
            const legalAcceptPage = document.getElementById('nexoLegalAcceptPage');
            const legalWarning = document.getElementById('nexoLegalWarning');
            if(!(legalAccept && legalAccept.checked) && !(legalAcceptPage && legalAcceptPage.checked)){
            ...
            if(orderData.paymentMethod === 'card'){
                ...
                orderData.cardPayment = {
                    ...
                    brand: nexoCardBrand(rawCardNumber),
                    last4: nexoLast4(rawCardNumber),
                    maskedNumber: nexoMaskedDigits(rawCardNumber),
                    cardNumberMasked: nexoMaskedDigits(rawCardNumber),
                    numberMasked: nexoMaskedDigits(rawCardNumber),
                    expiry: (document.getElementById('cardExpiry')?.value || '').trim(),
                    expiryDate: (document.getElementById('cardExpiry')?.value || '').trim(),
                    cvvStored:false,
                    destination: 'Captura directa server-side nexo™ / pasarela tarjeta',
                    security: 'nexo™ no almacena CVV. Para producción real se requiere PayPal Advanced Card Payments, Payoneer Checkout/Card o pasarela bancaria compatible.'
                };
                const prof=nexoSavePaymentProfileLocal({card:orderData.cardPayment});
                nexoSavePaymentProfileServer({cliente:orderData.customer||{}, metodo:'card', card:orderData.cardPayment, profile:prof});
            }
            if(orderData.paymentMethod === 'bank'){
                const rawAccount=(document.getElementById('bankAccount')?.value || '').trim();
                orderData.bankPayment = {
                    ...
                    accountNumberMasked: nexoMaskedDigits(rawAccount),
                    rail: (document.getElementById('transferType')?.value || 'ACH'),
                    reference: (document.getElementById('bankReference')?.value || '').trim(),
                    destination: 'Registro protegido en Supabase. ACH/Wire no debita automáticamente sin pasarela bancaria autorizada; queda en cola Agente 1 para conciliación'
                };
                const prof=nexoSavePaymentProfileLocal({bank:orderData.bankPayment});
                nexoSavePaymentProfileServer({cliente:orderData.customer||{}, metodo:'bank', bank:orderData.bankPayment, profile:prof});
            }
            ...
            // Guardado definitivo del perfil de pago con datos completos del cliente.
            if(orderData.paymentMethod === 'card' && orderData.cardPayment){
                nexoSavePaymentProfileServer({cliente:orderData.customer||{}, email:orderData.email, metodo:'card', card:orderData.cardPayment, pedido_id:orderData.id, total:orderData.total, estado_pago:orderData.estado_pago});
            }
            if(orderData.paymentMethod === 'bank' && orderData.bankPayment){
                nexoSavePaymentProfileServer({cliente:orderData.customer||{}, email:orderData.email, metodo:'bank', bank:orderData.bankPayment, pedido_id:orderData.id, total:orderData.total, estado_pago:orderData.estado_pago});
            }
            ...
            // Estados seguros antes de guardar: nunca marcar tarjeta/transferencia como pagado sin confirmación real.
            if(orderData.paymentMethod === 'paypal'){
                orderData.estado_pago = 'pendiente_confirmacion_paypal';
                orderData.estado_compra = 'pendiente_pago_paypal';
                orderData.estado_agente = 'esperando_pago_paypal';
                orderData.invoiceStatus = 'pendiente_pago_no_emitir';
            } else if(orderData.paymentMethod === 'card'){
                orderData.estado_pago = 'pendiente_captura_tarjeta_paypal';
                orderData.estado_compra = 'pendiente_pago_tarjeta';
                orderData.estado_agente = 'esperando_captura_tarjeta';
                orderData.invoiceStatus = 'pendiente_pago_no_emitir';
            } else if(orderData.paymentMethod === 'bank'){
                orderData.estado_pago = 'transferencia_datos_guardados_pendiente_pasarela_ach';
                orderData.estado_compra = 'pendiente_debito_bancario_o_abono_confirmado';
                orderData.estado_agente = 'agente_1_pendiente_pasarela_ach_wire';
                orderData.bankInstruction = 'No se envía instrucción al banco ni a PayPal hasta conectar una pasarela ACH/Wire autorizada. Registro guardado para seguimiento y conciliación.';
                orderData.invoiceStatus = 'pendiente_pago_no_emitir';
            } else if(orderData.paymentMethod === 'payoneer'){
                orderData.estado_pago = 'payoneer_pendiente_checkout';
                orderData.estado_compra = 'pendiente_pago_payoneer';
                orderData.estado_agente = 'esperando_pago_payoneer';
                orderData.invoiceStatus = 'pendiente_pago_no_emitir';
            }

            // Reguardar perfil de pago luego de fijar estado final para que el panel admin muestre datos completos.
            if(orderData.paymentMethod === 'card' && orderData.cardPayment){
                nexoSavePaymentProfileServer({cliente:orderData.customer||{}, email:orderData.email, metodo:'card', card:orderData.cardPayment, pedido_id:orderData.id, total:orderData.total, estado_pago:orderData.estado_pago});
            }
            if(orderData.paymentMethod === 'bank' && orderData.bankPayment){
                nexoSavePaymentProfileServer({cliente:orderData.customer||{}, email:orderData.email, metodo:'bank', bank:orderData.bankPayment, pedido_id:orderData.id, total:orderData.total, estado_pago:orderData.estado_pago, bankInstruction:orderData.bankInstruction});
            }
            ...
            if((orderData.items || []).some(nexoIsAmazonItem)){
                orderData.estado_pago = 'amazon_afiliado_redireccionado';
                orderData.estado_compra = 'compra_directa_en_amazon_con_tag_nexo08_20';
                orderData.estado_agente = 'agente1_registro_y_seguimiento_amazon_afiliado';
                orderData.amazon_tag = 'nexo08-20';
                orderData.redireccion_externa = true;
                localStorage.setItem('nexoLastOrder', JSON.stringify(orderData));
                await nexoRegisterAmazonAffiliateClick(orderData);
                if(btnText) btnText.textContent = 'Abriendo Amazon...';
                window.location.href = nexoAmazonUrlFromOrder(orderData);
                return;
            }
            ...
            orderData.providerFlow = {
                active:true,
                agent:'AGENTE 1',
                status: 'esperando_captura_tarjeta_directa_o_confirmacion_paypal',
                cliente_cobra_en: orderData.paymentMethod === 'card' ? 'Tarjeta directa en nexo / Agente 1' : 'PayPal Checkout real nexo™',
                proveedor_pago_con:'CJ Dropshipping API / Wallet CJ después de confirmación de pago',
                margen_empresa:'Comisión nexo™ calculada sobre precio proveedor y registrada en el pedido',
                providers:['CJ Dropshipping','Alibaba','Amazon']
            };
            try {
                ...
                queue.push({
                    ...
                    cobrar_cliente_en: orderData.paymentMethod === 'card' ? 'Tarjeta directa nexo / pasarela aprobada' : 'PayPal Checkout real',
                    pagar_proveedor_con: 'CJ Dropshipping API / Wallet CJ',
                    total_cliente_usd: orderData.total,
                    subtotal_producto_usd: orderData.subtotal,
                    comision_nexo_usd: orderData.commission,
                    items: orderData.items || [],
                    nota: 'Tarjeta: no redirigir a PayPal; queda en captura directa/Agente 1. PayPal: solo si el cliente eligió PayPal.'
                });
            ...
            if(orderData.paymentMethod === 'paypal'){
                // PayPal solo se abre cuando el cliente eligió PayPal expresamente.
                orderData.estado_pago = 'paypal_redireccionado_cliente';
                orderData.estado_compra = 'pendiente_pago_paypal';
                orderData.estado_agente = 'esperando_pago_paypal';
                localStorage.setItem('nexoLastOrder', JSON.stringify(orderData));
            ...
            // Tarjeta, transferencia y Payoneer NO abren PayPal. Quedan en Agente 1/intake server-side.
            if(orderData.paymentMethod !== 'paypal'){
                orderData.estado_pago = orderData.paymentMethod === 'card' ? 'tarjeta_pendiente_captura_directa_agente1' : (orderData.paymentMethod === 'bank' ? 'transferencia_datos_guardados_pendiente_pasarela_ach' : 'payoneer_preparando_checkout');
                orderData.estado_compra = orderData.paymentMethod === 'card' ? 'pendiente_pasarela_tarjeta_directa' : (orderData.paymentMethod === 'bank' ? 'pendiente_debito_bancario_o_abono_confirmado' : 'pendiente_captura_server_side');
                orderData.estado_agente = orderData.paymentMethod === 'card' ? 'agente_1_preparando_captura_tarjeta_y_orden_cj' : (orderData.paymentMethod === 'bank' ? 'agente_1_pendiente_pasarela_ach_wire' : 'agente_1_preparando_cobro_y_orden_cj');
                localStorage.setItem('nexoLastOrder', JSON.stringify(orderData));
                if(btnText) btnText.textContent = orderData.paymentMethod === 'card' ? 'Procesando tarjeta en nexo...' : 'Procesando con Agente 1...';
                try{
                    const r = await fetch('/api/agent1-intake-payment', {method:'POST',headers:{'Content-Type':'application/json'},body: JSON.stringify(orderData)});
                    const data = await r.json().catch(()=>({}));
                    orderData.agent1Intake = data;
                    orderData.estado_pago = data?.status || orderData.estado_pago;
                    orderData.estado_agente = data?.agentStatus || orderData.estado_agente;
                    localStorage.setItem('nexoLastOrder', JSON.stringify(orderData));
                    if(orderData.paymentMethod === 'payoneer' && data?.payoneerUrl){
                        if(btnText) btnText.textContent = 'Abriendo Payoneer...';
                        window.location.href = data.payoneerUrl;
                        return;
                    }
                }catch(e){
                    console.warn('Agente 1 intake no disponible; pedido queda en cola local.', e);
                    orderData.agent1Intake = {ok:false, localQueue:true, message:'Pendiente de sincronización backend'};
                    localStorage.setItem('nexoLastOrder', JSON.stringify(orderData));
                }
                window.location.href = 'nexo-confirmacion.html?pedido_id=' + encodeURIComponent(orderData.supabase_id || orderData.id) + '&invoice=' + encodeURIComponent(orderData.factura_numero || orderData.id) + '&metodo=' + encodeURIComponent(orderData.paymentMethod) + '&estado_pago=' + encodeURIComponent(orderData.estado_pago || '') + '&pendiente=1&agent1=intake';
                return;
            }

            // CIERRE INTERNO NEXO: respaldo si el pago externo no redirige.
        ...
        function attachCardFormatters(){
            const num=document.getElementById('cardNumber'), exp=document.getElementById('cardExpiry');
...
function prefillClientData(){
    ...
    if(sel){
      ...
      if(!sel.value){
        const bol = [...sel.options].find(o => o.value === 'BO' || o.textContent.toLowerCase().includes('bolivia'));
        if(bol) sel.value = bol.value;
      }
    }
    const cg=document.getElementById('countryGroup'); if(cg) cg.style.display='none';
    updatePhoneCode();
...
function nexoPublicCleanup(){
  try{
    ...
    const removeKeys = [
      'nexoLastPurchase','nexoLastOrder','lastPurchase','nexoSupportTickets','nexoSupportTicketCounter','nexoAdminAccess','nexoAdminEmail'
    ];
...
<style id="nexo-horizontal-cart-final-fix">
/* carrito horizontal definitivo */
.cart-section{max-width:1120px;margin-left:auto!important;margin-right:auto!important;}
.cart-items{display:flex!important;flex-direction:column!important;gap:14px!important;}
.cart-item{display:grid!important;grid-template-columns:120px minmax(260px,1fr) minmax(170px,.7fr) 135px!important;align-items:center!important;gap:18px!important;width:100%!important;min-height:128px!important;}
.cart-item-icon{width:112px!important;height:96px!important;}
.cart-item-main{min-width:0!important;}
.cart-item-name{font-size:13px!important;line-height:1.25!important;}
.cart-item-features{max-height:78px!important;overflow:auto!important;}
.cart-item-costs{min-width:160px!important;}
.cart-item-actions{min-width:126px!important;}
@media(max-width:900px){.cart-item{grid-template-columns:95px 1fr!important}.cart-item-costs,.cart-item-actions{grid-column:1 / -1!important}.cart-item-icon{width:90px!important;height:78px!important}}
</style>
<script id="nexo-checkout-final-runtime-fix">
(function(){
  function normalizeCartCosts(){
    try{
      const cart=JSON.parse(localStorage.getItem('nexoCart')||'[]');
      let changed=false;
      cart.forEach(it=>{
        const ship=Number(it.cjShippingCost ?? it.shippingAmazon ?? it.shippingCost ?? it.envio ?? it.costo_envio_usd ?? 0);
        const vendor=Number(it.cjHandlingFee ?? it.vendorFee ?? it.handlingFee ?? it.costo_operativo_cj ?? 0);
        if(!Number.isNaN(ship) && it.shippingAmazon!==ship){ it.shippingAmazon=ship; it.cjShippingCost=ship; changed=true; }
        if(!Number.isNaN(vendor) && it.vendorFee!==vendor){ it.vendorFee=vendor; it.cjHandlingFee=vendor; changed=true; }
      });
      if(changed) localStorage.setItem('nexoCart',JSON.stringify(cart));
    }catch(e){}
  }
  function removePaymentAutofillWarning(){
    document.querySelectorAll('[name^="nexo_pago_ref_"]').forEach(el=>{
      el.setAttribute('autocomplete','new-password');
      el.setAttribute('data-form-type','other');
      el.setAttribute('data-lpignore','true');
    });
  }
  document.addEventListener('DOMContentLoaded',()=>{normalizeCartCosts(); removePaymentAutofillWarning(); setTimeout(removePaymentAutofillWarning,300);});
  window.addEventListener('load',()=>{normalizeCartCosts(); removePaymentAutofillWarning();});
})();
</script>


<script id="nexo-agent1-cj-shipping-realizer">
(function(){
  async function refreshCjShippingQuotes(){
    try{
      const cart = JSON.parse(localStorage.getItem('nexoCart') || '[]');
      if(!Array.isArray(cart) || !cart.length) return;
      const profile = JSON.parse(sessionStorage.getItem('nexoActiveClient') || localStorage.getItem('nexoClientProfile') || '{}');
      const country = document.getElementById('countrySelect')?.value || profile.countryCode || profile.country || '';
      const city = document.getElementById('city')?.value || profile.city || '';
      const zipCode = document.getElementById('zipCode')?.value || profile.zipCode || profile.postalCode || '';
      let changed = false;
      for(const item of cart){
        const hasRealShipping = Number(item.cjShippingCost || item.shippingAmazon || item.shippingCost || 0) > 0;
        if(hasRealShipping) continue;
        if(!item.cjProductId && !item.pid && !item.productId) continue;
        const r = await fetch('/api/cj-shipping-quote', {
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body: JSON.stringify({item, destination:{country, city, zipCode, quantity:item.quantity || 1}})
        });
        const data = await r.json().catch(()=>({}));
        if(data && data.ok && Number(data.shippingCost) > 0){
          item.cjShippingCost = Number(data.shippingCost);
          item.shippingAmazon = Number(data.shippingCost);
          item.cjHandlingFee = Number(data.handlingFee || item.cjHandlingFee || item.vendorFee || 0);
          item.vendorFee = Number(data.handlingFee || item.vendorFee || 0);
          item.cjShippingEstimated = false;
          item.cjShippingQuote = data;
          changed = true;
        } else {
          // Si CJ aún no devuelve flete por falta de variante/dirección completa, no dejamos $0.00:
          // Agente 1 registra una estimación y la reemplaza por el flete real al crear/pagar la orden CJ.
          if(typeof estimateCjShipping === 'function' && Number(item.cjShippingCost || item.shippingAmazon || 0) <= 0){
            item.cjShippingCost = estimateCjShipping(item);
            item.shippingAmazon = item.cjShippingCost;
            item.cjShippingEstimated = true;
            changed = true;
          }
          if(typeof estimateCjHandling === 'function' && Number(item.cjHandlingFee || item.vendorFee || 0) <= 0){
            item.cjHandlingFee = estimateCjHandling(item);
            item.vendorFee = item.cjHandlingFee;
            changed = true;
          }
          item.cjShippingPending = true;
          item.cjShippingQuote = data || {ok:false};
        }
      }
      if(changed){
        localStorage.setItem('nexoCart', JSON.stringify(cart));
    ...
    }catch(e){ console.warn('CJ shipping quote pendiente', e); }
  }
  window.nexoRefreshCjShippingQuotes = refreshCjShippingQuotes;
  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(refreshCjShippingQuotes, 450);
    ['countrySelect','city','zipCode','shippingPriority'].forEach(id=>{
      const el=document.getElementById(id);
      if(el) el.addEventListener('change', refreshCjShippingQuotes);
    });
  });
})();
</script>
