 ...
 document.querySelectorAll('.payment-method, .payment-btn').forEach(btn=>{
   ...
   if(raw.includes('tarjeta')||raw.includes('card')||raw.includes('credito')) btn.innerHTML='💳 '+({es:'Tarjeta de Crédito/Débito',en:'Credit/Debit Card',pt:'Cartão de Crédito/Débito',it:'Carta di Credito/Debito',fr:'Carte de Crédit/Débit'}[currentLang]||'Credit/Debit Card');
        ...
        const paymentMethods = [
            ...
            { id: 'bank', name: 'Transferencia ACH / Wire', icon: '🏦' },
...
function selectPayment(methodId, btnEl=null) {
    ...
    if (methodId === 'card') {
        ...
        function calculateTotals() {
            ...
            const vendorFees = cart.reduce((total, item) => total + ((item.vendorFee || 0) * item.quantity), 0);
...
function openConfirmationModal() {
            ...
            const vendorFees = cart.reduce((sum, item) => sum + ((item.vendorFee || 0) * item.quantity), 0);
            const companyInfo = JSON.parse(localStorage.getItem('nexoCompanyInfo') || '{}');
            const commissionRate = typeof companyInfo.commission === 'number' ? companyInfo.commission / 100 : (parseFloat(localStorage.getItem('nexoCommission') || '30') / 100);
            const commission = subtotal * commissionRate;
            const itfRate = typeof companyInfo.itf === 'number' ? companyInfo.itf / 100 : 0.03;
            ...
            orderData = { fullName, email, phone: `${phoneCode} ${phoneNumber}`, country: country.options[country.selectedIndex].text, region, city, address, zipCode, paymentMethod: selectedPayment, shippingPriority, deliveryEstimate: deliveryInfo.text, deliveryLabel: deliveryInfo.label, items: cart, itemsCount: cart.reduce((n,i)=>n+(i.quantity||1),0), subtotal, amazonShipping, vendorFees, commission, itf, iue, total };
...
const t = ctt();
            ...
            document.getElementById('confirmationSummary').innerHTML = `<strong>${t.confirmClient}:</strong> ${fullName}<br><strong>${t.confirmEmail}:</strong> ${email}<br><strong>${t.confirmPhone}:</strong> ${phoneCode} ${phoneNumber}<br><strong>${t.confirmDestination}:</strong> ${country.options[country.selectedIndex].text}, ${region}, ${city}<br><strong>${t.confirmAddress}:</strong> ${address} | CP ${zipCode}<br><strong>${t.confirmPayment}:</strong> ${paymentLabel}<hr style="border:none;border-top:1px solid #d8e5e2;margin:12px 0"><strong>${t.confirmSubtotal}:</strong> $${subtotal.toFixed(2)}<br><strong style="color:#f39c12;">${t.confirmShip}:</strong> <span style="color:#f39c12;">$${amazonShipping.toFixed(2)}</span><br><strong style="color:#f39c12;">${t.confirmVendor}:</strong> <span style="color:#f39c12;">$${vendorFees.toFixed(2)}</span><br><strong style="color:#f39c12;">${t.deliveryOrange}:</strong> <span style="color:#f39c12;">${deliveryInfo.label} · ${deliveryInfo.text.replace(/^.*?: /,'')}</span><br><strong>${t.confirmCommission}:</strong> $${commission.toFixed(2)}<br><strong style="font-size:18px;color:#7ac742;">${t.confirmTotal}: $${total.toFixed(2)}</strong>`;
        ...
        async function submitPurchase(){
            ...
            orderData.providerFlow = {
                active:true,
                agent:'AGENTE 1',
                status:'cola_pago_proveedor',
                cliente_cobra_en:'PayPal nexo™',
                proveedor_pago_con:'PayPal (si el proveedor acepta PayPal)',
                margen_empresa:'Diferencia retenida en PayPal como comisión nexo™',
            ...
            try {
                ...
                queue.push({
                    ...
                    cobrar_cliente_en: orderData.paymentMethod === 'bank' ? 'Banco/ACH/Wire vía PayPal' : 'PayPal',
            ...
            if (['paypal','card','bank'].includes(orderData.paymentMethod)) {
                try {
                    if(btnText) btnText.textContent = 'Abriendo pago seguro...';
                    await startNexoPagoSeguro(orderData);
                    return;
                } catch(payErr) {
                    console.warn('PayPal no abrió por error inesperado.', payErr);
                    alert('No se pudo abrir PayPal automáticamente. Revise que el navegador permita redirecciones y vuelva a confirmar.');
                    if(btnText) btnText.textContent = oldBtnText || 'Confirmar compra';
                    return;
                }
            }
            localStorage.removeItem('nexoCart');
            window.location.href = 'nexo-confirmacion.html';
        }
